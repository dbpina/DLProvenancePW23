...
def main(opt):
	...
	class2,class1 = df['Class'].value_counts()	

	c1 = df.loc[df["Class"] == 1]
	c2 = df.loc[df["Class"] == 0]

	df_1 = c1.sample(class2, replace=True)

	oversampled_df = pd.concat([c2,df_1], axis=0)
