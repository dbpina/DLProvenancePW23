...
class Provenance:
    ...
    def __init__(self, df, results_path=None):
        ...
        self.new_entities = []
        self.debora_entities = []   
        self.new_debora_entities = []   
        #self.chunk_ent = round(math.ceil(len(df.index)/4000))
        self.debora_records = []  
        self.data_types = None
        self.columnsToDelete = []
    ...
    def create_entity_debora(self, record_id, dframe, mydtypes):
        """Create a provenance entity.
        Return a dictionary with the id and the attributes of the entity."""
        # Get attributes:
        other_attributes = {}
        ...
        #for i in range(len(dframe.keys())):
        #    if mydtypes[dframe.keys()[i]] == "object":
        #        other_attributes[dframe.keys()[i]] = str(dframe[i])    
        #    else:
        #        other_attributes[dframe.keys()[i]] = str(dframe[i].astype(mydtypes[dframe.keys()[i]]))
            
        for i in dframe.keys():
            if mydtypes[i] == "object":
                other_attributes[i] = dframe[i]
            else:
                other_attributes[i] = str(dframe[i].astype(mydtypes[i]))

        ent = other_attributes
        self.debora_entities.append(ent)
        self.debora_records.append(record_id)

    def create_entity_update(self, record_id, value, feature_name, data_type):
        if record_id in self.debora_records:
            if data_type == "float64":
                self.debora_entities[self.debora_records.index(record_id)][feature_name] = str(value)
            elif data_type == "int64":
                self.debora_entities[self.debora_records.index(record_id)][feature_name] = str(value)
            else:
                self.debora_entities[self.debora_records.index(record_id)][feature_name] = value
            
        else: 
            if data_type == "float64":
                self.debora_entities.append({'record_id': record_id, feature_name:str(value.astype(data_type))})
            else:
                self.debora_entities.append({'record_id': record_id, feature_name:str(value)})
            self.debora_records.append(record_id)  
    ...
    def create_prov_entities(self, dataframe, instance=None):
        ...
        self.data_types = dict(dataframe.dtypes)
        ...
        self.debora_entities.clear()
        for i in range(self.current_m):
            record_id = str(uuid.uuid4())
            self.create_entity_debora(record_id, dataframe.iloc[i], self.data_types)
    ...
    def get_prov_feature_transformation(self, df_out, columnsName, description=None):
        ...
        for i in range(self.current_m):
            line = entities_in[i]
            record_id = list(line)[0]['attributes']['record_id']
            the_record_id = self.debora_records.index(record_id)
            for col_name in columnsName:
                ...
                if col_name is 'sex' and value != 'Male' and value != 'Female':
                    self.debora_entities[the_record_id][col_name] = str(value)
                else:
                    self.debora_entities[the_record_id][col_name] = value
    ...
    def get_prov_space_transformation(self, df_out, columnsName, description=None):
        ...
        self.initialize()
        #self.columnsToDelete.append(columnsName[0])
        # Get current values:
        entities_in = self.current_ent
        ...
        for i in range(m):
            ...
            the_record_id = self.debora_records.index(record_id)           
            del self.debora_entities[the_record_id][columnsName[0]]
            for j in indexes_new:
                ...
                self.debora_entities[the_record_id][columns_out[j]] = str(value)
                e_out_identifier = e_out['identifier']
    ...
    def get_prov_dim_reduction(self, df_out, description=None):
        ...
        for i in delIndex:
            #print(self.debora_entities[i]['record_id'])
            #del self.debora_entities[i]
            for j in range(n):
                e_in = entities_in[i][j]
                e_in_identifier = e_in['identifier']
                e_in_feature = e_in['attributes']['feature_name']
                del self.debora_entities[i][e_in_feature]
                self.create_relation(self.INVALIDATION, a=e_in_identifier, b=act_id)
        ...
        for colName in delColumnsName:
            ...
            delColumns.append(j)
            for deb in self.debora_entities:
                del deb[colName]
            for i in range(m):
                e_in = entities_in[i][j]
                e_in_identifier = e_in['identifier']
    ...
    def get_prov_instance_generation(self, df_out, columnsName, description=None):
        ...
        for i in range(m, m_new):
            ...
            for j in range(n):
                ...
                ent_id = self.NAMESPACE_ENTITY + str(uuid.uuid4())
                e_out = self.create_entity(ent_id, record_id, value, col_name, indexes_out[i], self.operation_number)
                self.create_entity_update(record_id, value, col_name)
                e_out_identifier = e_out['identifier']
        ...
        for col_name in columnsName:
            ...
            for i in range(self.current_m):
                ...
                if str(val_in) != str(value):
                    ...
                    ent_id = self.NAMESPACE_ENTITY + str(uuid.uuid4())
                    e_out = self.create_entity(ent_id, record_id, value, col_name, indexes_out[i], self.operation_number)
                    self.create_entity_update(record_id, value, col_name, self.data_types[col_name])
                    e_out_identifier = e_out['identifier']
                    self.create_relation(self.GENERATION, a=e_out_identifier, b=act_id)
                    self.create_relation(self.USE, a=act_id, b=e_in_identifier)
                    self.create_relation(self.DERIVATION, a=e_out_identifier, b=e_in_identifier)
    ...
    @timing
    def save_debora_entities(self, debora_path, debora_file):
        #print(self.columnsToDelete)
        #print(self.debora_entities)
        # my_hash = []
        # for item in self.debora_entities:
        #     record_id = item['record_id']
        #     del item["record_id"]
        #     del item["label"]
        #     the_hash = hashlib.sha256(str(item).encode("utf-8")).hexdigest()               
        #     #print(the_hash)
        #     #print(item)
        #     my_hash.append({"record_id": record_id, "hash": the_hash})
        #print(my_hash)
        with open(debora_path + "/" + debora_file + ".json", 'w', encoding='utf-8') as sample_file:
            json.dump(self.debora_entities, sample_file, ensure_ascii=False, indent=4)   
        # with open(debora_path + "/" + debora_file + ".json", 'w', encoding='utf-8') as sample_file:
        #     json.dump(my_hash, sample_file, ensure_ascii=False, indent=4)                    
