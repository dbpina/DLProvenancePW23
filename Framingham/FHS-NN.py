...
def save_records():
  ...
  for j in len(x_train):
    my_list = []
    values_attrs = x_train.iloc[j]
    for i in range(0,len(attrs),1):
      my_list.append({str(attrs[i]): str(values_attrs[i])})
    cursor = list(db.debora_data.find({'$and':my_list}))
    record_ids.append(cursor[0]['record_id'])
  query = ("INSERT INTO record_tuples(record_id,record_type) VALUES ((%s,'train');")
  cursor.executemany(query, records_ids)

  record_ids = []
  for k in len(x_test):
    my_list = []
    values_attrs = x_test.iloc[k]
    for i in range(0,len(attrs),1):
      my_list.append({str(attrs[i]): str(values_attrs[i])})
    cursor = list(db.debora_data.find({'$and':my_list}))
    records_ids.append(cursor[0]['record_id'])
  ...
  #cursor = connection.cursor()
  #query = ("INSERT INTO record_tuples(record_id,record_type) VALUES (%s,'train');")
  #cursor.executemany(query, cursor_mongo)
  #connection.commit()
  #connection.close()
