...
def save_records():
  ...
  def teste(number):
    my_list = []
    for i in range(0,len(number.index),1):
      if (number.index[i] == 'male' or number.index[i] == "age" or number.index[i] == "currentSmoker" or number.index[i] == "prevalentStroke" or number.index[i] == "prevalentHyp" or number.index[i] == "diabetes"):
        my_list.append({str(number.index[i]): str(int(number.values[i]))})
      else:
        my_list.append({str(number.index[i]): str(number.values[i])})
    cursor_mongo = list(db.debora_data.find({'$and':my_list}))
    record_ids.append(cursor_mongo[0]['record_id'])

  x_train.apply(teste,axis=1)
  query = ("INSERT INTO record_tuples(record_id,record_type) VALUES (%s,'train');")
  cursor.executemany(query, record_ids)

  record_ids = []
  x_test.apply(teste,axis=1)
